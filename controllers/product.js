...
exports.createProduct = (req, res) => {
  ...
  form.parse(req, (err, fields, file) => {
    if (err) {
      return res.status(400).json({ error: "Couldn't store into the DB" });
    }
...
exports.updateProduct = (req, res) => {
  ...
  form.parse(req, (err, fields, file) => {
    if (err) {
      return res.status(400).json({ error: "Couldn't update into the DB" });
    }
...
exports.getAllUniqueCategories = (req, res) => {
  Product.unique("cataegory", {}, (err, obj) => {
    if (err) {
      return res.status(400).json({ error: "Couldn't get any categories :(" });
    }
    return res.json(obj);
  });
};
